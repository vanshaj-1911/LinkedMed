...
export default function VideoCall() {
  ...
  useEffect(() => {
    ...
    // Dynamically import socket.io-client to avoid top-level crash
    let mounted = true
    ...
    const init = async () => {
      ...
      socket.on('chat-message', msg => {
        if (mounted) setChatMsgs(prev => [...prev, msg])
      })
    }
  ...
  const sendChat = (e) => {
    ...
    setChatMsgs(prev => [...prev, { message: chatInput, senderName: 'You', time: new Date().toLocaleTimeString() }])
    setChatInput('')
  }
    ...
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', background:'#0f172a', color:'#fff' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 24px', background:'rgba(0,0,0,0.4)', borderBottom:'1px solid rgba(255,255,255,0.1)', flexWrap:'wrap', gap:'8px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'14px', flexWrap:'wrap' }}>
          <span style={{ fontSize:'18px', fontWeight:'800', color:'#38bdf8' }}>🏥 TeleMed</span>
      ...
      <div style={{ flex:1, position:'relative', overflow:'hidden', background:'#1e293b' }}>
        ...
        {/* Local video PIP */}
        <div style={{ position:'absolute', top:'16px', right:'16px', width:'200px', height:'150px', borderRadius:'14px', overflow:'hidden', border:'2px solid rgba(255,255,255,0.2)', boxShadow:'0 8px 32px rgba(0,0,0,0.5)' }}>
        ...
        {showChat && (
          <div style={{ position:'absolute', top:0, right:0, width:'280px', height:'100%', background:'rgba(15,23,42,0.97)', display:'flex', flexDirection:'column', borderLeft:'1px solid rgba(255,255,255,0.1)' }}>
            <div style={{ padding:'16px', fontWeight:'700', fontSize:'15px', borderBottom:'1px solid rgba(255,255,255,0.1)' }}>💬 In-Call Chat</div>
            <div style={{ flex:1, overflowY:'auto', padding:'12px', display:'flex', flexDirection:'column', gap:'10px' }}>
              {chatMsgs.length === 0 && <p style={{ color:'#475569', fontSize:'14px', textAlign:'center', marginTop:'20px' }}>No messages yet</p>}
              {chatMsgs.map((m, i) => (
                <div key={i} style={{ background: m.senderName==='You' ? 'rgba(14,165,233,0.25)' : 'rgba(255,255,255,0.07)', borderRadius:'10px', padding:'10px', display:'flex', flexDirection:'column', gap:'4px' }}>
            ...
            <form onSubmit={sendChat} style={{ display:'flex', padding:'12px', gap:'8px', borderTop:'1px solid rgba(255,255,255,0.1)' }}>
              ...
              <button type="submit" style={{ background:'#0ea5e9', color:'#fff', border:'none', borderRadius:'8px', padding:'8px 14px', cursor:'pointer', fontWeight:'600', fontSize:'13px' }}>Send</button>
            </form>
          </div>
        )}
      </div>
